...
include_once('../config.php');
    ...
    $sqlCPF = "SELECT * FROM usuarios ORDER BY id DESC limit 30";
    $resultCPF = $conn->query($sqlCPF);

    //Ligação bd do termos:
    $sqlTermo = "SELECT * FROM termos";
    $resultTermo = $conn->query($sqlTermo);
    $user_data = mysqli_fetch_assoc($resultTermo);
    ...
    <div class="main-content">
            ...
            <main class="principal-painel">
                <div class="lista-usuarios">
                    <div class="header-usuarios mt-3 d-flex flex-column flex-sm-row justify-content-around">
                        <div class="titulo-usuarios mt-3">
                            <h2 class="title mb-5 text-center"><i class="far fa-newspaper"></i> Upload dos termos:</h2>
                        </div><!--titulo-usuarios-->
                    </div><!--header-usuarios-->
                    <div class="termos-upload">
                        <form action="termoSave.php" enctype="multipart/form-data" method="post" class="termo-edit">
                            <div class="row">
                                <div class="col-md-8 mb-3">
                                    <label for="nomeTermo" class="form-label">Envie um arquivo</label>
                                    <input type="file" class="form-control" name="termo" id="nomeTermo" aria-describedby="termoHelp" required>
                                    <div id="termoHelp" class="form-text">Edite o termo de corretores aqui.</div>
                                </div><!--campo-->
                                <div class="col-md-4 mt-4">
                                    <input type="submit" value="Editar Termo" class="btn input-logar" name="editar_termo">
                                </div><!--campo-->
                            </div><!--row-->
                        </form><!--termo-edit-->
                        <p class="text-left">Termo atual:</p>
                        <table class="table table-usuarios bg-table">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td><?php echo $user_data['termo'] ?></td>
                                    <td><?php echo $user_data['path_termo'] ?></td>
                                    <td><a class="btn btn-primary" target="_blank" href="<?php echo $user_data['path_termo']; ?>"><i class="far fa-eye"></i></a></td>
                                </tr>
                            </tbody>
                        </table>
                    </div><!--termos-upload-->
